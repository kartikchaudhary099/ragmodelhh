...
class SentenceChunker(BaseChunker):
    ...
    def _build_chunk(record: dict[str, Any], text: str, index: int) -> Chunk:
        ...
        parent_id = record.get("metadata", {}).get("parent_id") if isinstance(record.get("metadata"), dict) else None
        if parent_id:
            metadata["parent_id"] = parent_id
        provenance = record.get("metadata", {}) if isinstance(record.get("metadata"), dict) else {}
        metadata.update({k: v for k, v in provenance.items() if k not in {"passage", "query"}})
        return Chunk(
            text=text.strip(),
    ...
    def _resolve_text(record: dict[str, Any]) -> str:
        if not record:
            return ""
        text = record.get("passage") or record.get("metadata", {}).get("passage") or record.get("query") or ""
        return str(text).strip()
